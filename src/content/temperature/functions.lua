...
function OPAL.get_temp_colour()
    ...
    if G.GAME.opal_temperature < 10 then
        _colour = (mix_colours(G.C.RED, G.C.WHITE, to_number(math.max(0,math.min((G.GAME.opal_temperature)/10,1)))))
    elseif G.GAME.opal_temperature < 25 then
        _colour = (mix_colours(G.C.FILTER, G.C.RED, to_number(math.max(0,math.min((G.GAME.opal_temperature-10)/15,1)))))
    elseif G.GAME.opal_temperature < 50 then
        _colour = (mix_colours(G.C.BLUE, G.C.FILTER, to_number(math.max(0,math.min((G.GAME.opal_temperature-25)/25,1)))))
    else
        _colour = (mix_colours(mix_colours(G.C.PURPLE, G.C.WHITE, to_number(0.7)), G.C.BLUE, to_number(math.max(0,math.min((G.GAME.opal_temperature-50)/25,1)))))
    end
    return(_colour)
end
